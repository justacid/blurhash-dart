...
class BlurHash {
  ...
  /// Creates a String from Components. The result differs slightly from the correct result, if the BlurHash
  /// was generated from a String or an Image, but is correct if the BlurHash was generated by a blend or mirror function
  /// So it should only be called, when an Image was generated or mirrored
  String createString(){
...
int numCompY = 3,
  ...
  final genBlurHash = fromString(blurHash.toString());
/*
  final reduced = List<Color>(numCompX * numCompY);
  var count = 0;
  for(var j = 0; j < numCompY;j++){
    for(var i = 0; i < numCompX;i++){
      reduced[count++] = genBlurHash.components[j][i];
    }
  }
  for(var i=0;i<reduced.length;i++){
    print("r values: (${reduced[i].r})-(${factors[i].r}) = (${reduced[i].r - factors[i].r})");
    print("g values: (${reduced[i].g})-(${factors[i].g}) = (${reduced[i].g - factors[i].g})");
    print("b values: (${reduced[i].b})-(${factors[i].b}) = (${reduced[i].b - factors[i].b})");
   }
*/
  return genBlurHash;
}
...
BlurHash blendingTopBottom(topBlurHash, bottomBlurHash){
  if (topBlurHash.components.length != 1 && bottomBlurHash.components[0].length != 1) {
    throw BlurHashEncodeException(
...
BlurHash generateColoredBlurHash(List color){
  return BlurHash([[createColor(color)]]);
}

BlurHash generateBlendTopBottom(List topColor,List bottomColor){
...
BlurHash generateBlendCorners(List topLeft,List topRight,List bottomLeft,List bottomRight){
  return blendingTopBottom(transpose(blendingTopBottom(generateColoredBlurHash(topLeft),generateColoredBlurHash(topRight))), transpose(blendingTopBottom(generateColoredBlurHash(bottomLeft), generateColoredBlurHash(bottomRight))));
}

BlurHash generateColorSteps(List horizontalColors, numberOfComponents){
  ...
  for(var i = 0; i < numberOfComponents; i++){
    ...
    for(var x = 0; x < horizontalColors.length; x++){
      ...
      sum += createColor(horizontalColors[x]) * basis;
    }
...
Color createColor(List color){
  return Color(sRGBtoLinear(color[0]),sRGBtoLinear(color[1]),sRGBtoLinear(color[2]));
}
